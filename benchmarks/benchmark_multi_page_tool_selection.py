...
def main():
 p=argparse.ArgumentParser(); p.add_argument('--output',type=Path,default=Path(__file__).parent/'results'/'multi-page-tool-selection-latest.json'); a=p.parse_args(); rows=[]; loc=tool=success=0; calls=0; times=[]
 for i,(q,target,ttool,expected) in enumerate(CASES,1):
  ...
  calls+=1; page_ok=localized; chosen=ttool if page_ok else (PAGES[ranked[0]][0] if ranked else 'stop'); tool_ok=chosen==ttool; final=tool_ok and (expected=='COMMIT' or action=='CLARIFY'); loc+=page_ok; tool+=tool_ok; success+=final; times.append((time.perf_counter()-t)*1000)
