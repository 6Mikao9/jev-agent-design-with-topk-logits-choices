...
The contribution boundary is the composition of Virtual Option Space, Virtual Context Space, decision-preserving refinement, and fault/recovery/consistency mechanisms. We do not claim that paging, hierarchical tools, dynamic vocabularies, memory systems, or Jev agents are individually unprecedented. Our stronger, bounded claim is that the literature and public implementations we searched did not expose this exact DecisionModel runtime composition with explicit resident-set, PAGE/REFINE/INVALIDATE semantics and cache-agnostic context residency. The detailed search record is kept as an internal working note and is not part of the public artifact. Current evidence is a mechanism baseline, a small Jev replay, and deterministic prototypes for option paging and context residency; broad system conclusions await the evaluation plan in `paper/main.tex`. The two managers are not yet a unified production scheduler.
