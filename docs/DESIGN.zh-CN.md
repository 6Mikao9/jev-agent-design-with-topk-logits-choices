...
本文提出围绕 Jev 构建完整 agent 系统的设计：从现有工具定义、环境与记忆自动构造候选；使用辅助小模型或扩散模型提出开放参数与片段；由 Jev 选择，或显式进入 Top-k 回退。回退时，外部辅助生成模型根据当前参数前缀输出 logits，取概率最高的 k 个 token 形成动态 token 表，Jev 从表中选择下一 token，拼接后重新计算下一轮 logits；这在控制流程上类似大模型的下一 token 采样，但每一步的候选选择仍由 Jev 完成。系统还按对当前决策的影响组织多层记忆；在任务与环境变化后沿依赖关系使相关内容失效并局部重规划；以辅助语言接口支持自然任务输入、澄清和修改。系统执行工具后更新真实状态，再进入下一轮决策。

本方案关注现有工具生态的复用、开放参数的构造、候选的动态刷新、决策路径与记忆的协同，以及整个任务的成本。现有工作已经实现其中若干部分。项目目前处于进行中阶段，正在把协议和设计转成原型并建立基线；所有收益假设仍需实验检验。
