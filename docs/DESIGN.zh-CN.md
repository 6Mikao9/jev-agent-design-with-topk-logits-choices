...
本文提出围绕 Jev 构建完整 agent 系统的设计，并记录一个已经实现的自然语言对话原型：从现有工具定义、环境与记忆自动构造候选；使用可替换的外部生成模型提供开放参数与文本生成所需的下一 token logits；由 Jev 选择，或显式进入 Top-k 回退。回退时，外部辅助生成模型根据当前参数前缀输出 logits，取概率最高的 k 个 token 形成动态 token 表，Jev 从表中选择下一 token，拼接后重新计算下一轮 logits；这在控制流程上类似大模型的下一 token 采样，同时能够形成句子级、可正常交流的自然语言，而不是固定词表的字符或词拼接。每一步候选选择仍由 Jev 完成。系统还按对当前决策的影响组织多层记忆；在任务与环境变化后沿依赖关系使相关内容失效并局部重规划；以辅助语言接口支持自然任务输入、澄清和修改。系统执行工具后更新真实状态，再进入下一轮决策。

本方案关注现有工具生态的复用、开放参数的构造、候选的动态刷新、决策路径与记忆的协同，以及整个任务的成本。当前已经有一个可运行原型：外部生成模型根据当前文本前缀输出下一 token logits，Top-k 形成动态 token 表，Jev 从表中选择下一 token 并循环生成连贯文本。Qwen‑0.8B 是当前实验中的一个低成本实现选择，不是系统定义的一部分。项目目前处于进行中阶段，正在整理代码、建立基线并补充评估；所有收益假设仍需实验检验。

### 1.4 与固定词表生成的区别

固定词表方案把字符、词或短语预先写入选项，再让 Jev 重复选择。这种方式可以验证连续选择流程，但可能受到词表覆盖、未登录词、分词边界、组合效率和上下文连贯性的限制；词表没有覆盖的表达只能近似拼接，输出质量依赖词表设计。

本项目由外部生成模型根据已经选中的文本前缀动态计算 logits，并把概率最高的 token 组成当前轮 token 表。Jev 选择一个 token 后，系统把它加入前缀，再计算下一轮 token 表。候选因此来自语言模型的上下文分布，可以组合预先没有枚举的词、缩写和句子结构，目标是在保留 Jev 逐步决策的同时实现正常自然语言交流。流畅度、成本和错误率仍需通过公开代码和统一基线实测。
...
- 明确 Jev 逐轮选择下一 token 后重新计算前缀 logits，形成句子级自然语言输出。
- 将原型定位为“有界检索中首个公开 Jev + 外部 Qwen‑0.8B logits 流畅对话方案”的限定声明，不作全球首创保证。
