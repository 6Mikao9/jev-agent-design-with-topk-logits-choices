...
| Status | Core agent, TypeSafeJeV adapter, and token-by-token helper path implemented; live Jev long-form evaluation pending |
| Author | Anonymous |
...
The proposed system reuses tool schemas and executors, asks a small autoregressive model or a diffusion model to propose complete arguments, fields, or fragments, and lets Jev select among them. When coarse proposals are unsuitable and task information is sufficient, Jev can select an explicit `FALLBACK_TOPK` action. The helper model computes logits for the current prefix and the highest-probability `k` tokens form a dynamic token table; the designed path lets Jev choose a token, appends it to the prefix, and requests the next logits. The implemented code includes the core agent, a TypeSafeJeV adapter, and the token-by-token helper path. However, the four saved fluent-answer and speed runs used `local_top1_proxy`, which directly selects the helper's top-1 token at each step; they did not use live Jev for token-by-token selection. A live Jev smoke test covered tool-candidate selection only, so long-form fluency and answer quality with Jev remain unverified. The design also includes decision-impact hierarchical memory (I1), dependency-aware local replanning (I3), an independent candidate-coverage problem (I2), natural interaction, candidate refresh, and cost accounting.

The project is in progress. A working prototype provides a replaceable external-model helper path that computes next-token logits and exposes a dynamic Top-k token table. The saved multi-sentence answers were generated by `local_top1_proxy` (direct helper top-1 selection), not by Jev. The live Jev adapter has only been smoke-tested on tool-candidate selection; live Jev token-by-token long-form dialogue has not yet been evaluated. Qwen-0.8B is one low-cost implementation choice, not part of the system definition. Code cleanup, reproducibility, and baseline evaluation are underway. A bounded search has not established a global novelty claim, and no claim of demonstrated fluent dialogue with live Jev is made here.

### Implementation and evidence boundary

Implemented: core agent, TypeSafeJeV adapter, and token-by-token helper path. The four saved long-answer and speed experiments used `local_top1_proxy`, not live Jev decisions. One live smoke test covered tool-candidate selection only. Evidence for live Jev long-form generation, answer quality, and latency is pending; other features not explicitly marked implemented remain design proposals or evaluation targets.
...
The designed path asks an external language model to compute logits from the text prefix selected so far. The highest-probability tokens form the current token table; after the selector chooses one, it is appended to the prefix and the next table is computed. The candidates therefore follow a language model's contextual distribution and can compose words, abbreviations, and sentence structures that were not enumerated in advance. The goal is to keep Jev in every decision while supporting natural-language conversation. Live Jev fluency, cost, and error rates still require controlled evaluation.
