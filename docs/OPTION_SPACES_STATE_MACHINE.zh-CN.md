...
建议把 **2/4/8** 保留为小空间的快速梯度：先给 2 个，无法决定时扩到 4 个，再扩到 8 个；超过 8 个时进入分页或先做摘要筛选。对于需要对齐 Jev API 的大目录，逻辑目录页可以容纳最多 255 个条目，但不把它们默认全部送入一次决策。当前默认实际调用为 16 个动作候选加 6 个控制项，实验比较 8/16/32（必要时 64）。详见 [Option Space 页与调用预算](OPTION_PAGE_BUDGET.zh-CN.md)。
